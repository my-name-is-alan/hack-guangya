...
function folderName(value) { const name = String(value || '').trim().normalize('NFC'); return name && name !== '.' && name !== '..' && [...name].length <= 255 && !/[\\/:*?"<>|\u0000-\u001f\u007f-\u009f]/.test(name) ? name : ''; }
...
const server = http.createServer(async (request, response) => {
  ...
  if (request.method === 'POST' && url.pathname === '/api/hdhive/config') { const input = await body(request); state.hdhive = { ...state.hdhive, enabled: typeof input.enabled === 'boolean' ? input.enabled : state.hdhive.enabled !== false, configured: Boolean(input.base_url && input.secret), base_url: input.base_url || '' }; return json(response, 200, state.hdhive); }
  ...
  if (request.method === 'POST' && url.pathname === '/api/files/create-folder') { const input = await body(request); const parentId = String(input.parent_id || ''); const validFolderName = folderName(input.folder_name); const list = files.get(parentId) || []; if (!validFolderName || list.some((item) => item.fileName === validFolderName)) return json(response, 400, { error: !validFolderName ? '无效的文件夹名称' : '文件夹已存在' }); const folder = { fileId: `folder-${crypto.randomUUID()}`, fileName: validFolderName, resType: 2, subFolderCount: 0, utime: Math.floor(Date.now() / 1000) }; list.unshift(folder); files.set(parentId, list); files.set(folder.fileId, []); return json(response, 200, { fileId: folder.fileId }); }
