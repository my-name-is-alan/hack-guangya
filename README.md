...
- 当前账号开发者模式与小号秒传：在账号配置中填写当前登录账号自己的官方开发者 `client_id` / `client_secret`；所有权验证通过后，文件列表/详情可在主接口失败时使用开发者接口兜底，并可保存多个小号接收 TOKEN 进行秒传。
...
- 账户配置：展示账号昵称、账号 ID、手机号与空间，并在同一页管理严格绑定当前账号的开发者模式；不再展示 VIP/SVIP、流量和权益规则。
...
安装包：`target/release/bundle/nsis/光鸭文件夹同步_0.1.23_x64-setup.exe`
...
打开“设置 → 账号 → 开发者模式”，填写当前登录账号在光鸭开发者后台生成的 `client_id` 和 `client_secret`。点击“验证当前账号”时，应用先用当前登录态读取一个真实 `fileId`，再用开发者凭据读取同一个文件；两次都成功才记录账号绑定并允许打开开关。切换登录账号、修改 `client_id` 或验证状态失效时，开发者模式立即停用，避免浏览或转存其他账号的文件。

模式开启后，普通 PC 接口仍是文件列表与详情的主路径；只有主接口读取失败，才使用 `https://dapi.guangyapan.com` 的同名只读接口兜底。新建、移动、重命名、删除等未出现在开发者文档中的操作仍使用当前 PC 登录会话。Docker Web 也可以通过 `GUANGYA_DEVELOPER_CLIENT_ID`、`GUANGYA_DEVELOPER_CLIENT_SECRET` 注入凭据，但仍必须在账号页完成当前账号验证和模式启用。

小号在光鸭中创建接收 TOKEN，并给 TOKEN 授权一个目标目录；回到账号页添加该 TOKEN。之后在“文件”中选中最多 20 个文件或文件夹，点击“小号秒传”并选择接收小号。提交前应用会再次用开发者凭据确认所选文件属于已绑定账号，然后调用 `upload_by_fileid`；若文件尚未通过审核（业务码 `18011`），自动调用 `pre_upload`，后台轮询预审完成后继续秒传。任务及脱敏统计保存在本机 SQLite，应用重启后会恢复未完成任务；完整 `client_secret` 和接收 TOKEN 不通过状态接口回显。
...
```bash
docker pull 94xhzy/guangya-sync:0.1.22
```
