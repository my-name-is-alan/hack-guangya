...
打开 `http://localhost:8080`。默认挂载关系为：
...
输出位于 `release/guangya-sync-native-ubuntu-x64-0.1.13.tar.gz`，解压后执行 `sudo ./install.sh`。安装包自带 Node.js 24 Linux 运行时和全部生产依赖。Ubuntu 原生版的服务器文件选择器默认从 `/` 开始，实际可见范围由 `guangya-sync` 服务用户的系统权限决定；也可使用 `GUANGYA_FILE_ROOTS` 设置目录白名单。应用自己的 `DATA_DIR` 始终隐藏，避免误选并上传包含登录会话的状态库。详细说明见包内 `README.md`。
