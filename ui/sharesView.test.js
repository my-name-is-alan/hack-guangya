...
test('SharesView safely opens only HTTP share links in a new isolated context', async () => {
  const source = await sharesViewSource;
  const openSource = sourceBetween(source, 'function safeCloudShareUrl(record)', 'async function deleteShare(record)');

  assert.match(openSource, /new URL\(value\)/);
  assert.match(openSource, /\[['"]http:['"],\s*['"]https:['"]\]\.includes\(url\.protocol\)/);
  assert.match(openSource, /window\.open\(url,\s*['"]_blank['"],\s*['"]noopener,noreferrer['"]\)/);
  assert.match(source, /aria-label=['"]打开分享链接['"]/);
  assert.match(source, /@click=['"]openCloudShare\(record\)['"]/);
});

test('SharesView keeps the cloud share table fixed and actions inside narrow windows', async () => {
  const source = await sharesViewSource;

  assert.match(source, /class=['"]cloud-share-table['"][^>]*table-layout=['"]fixed['"]/);
  assert.match(source, /class=['"]share-name-cell['"]/);
  assert.match(source, /class=['"]share-actions['"]/);
  assert.match(source, /\.share-name-cell \.file-name[\s\S]*text-overflow:\s*ellipsis[\s\S]*white-space:\s*nowrap/);
  assert.match(source, /\.share-actions[^{]*\{[^}]*flex-wrap:\s*nowrap[^}]*white-space:\s*nowrap/);
  assert.match(source, /@media \(max-width:\s*860px\)/);
});
