...
const bridgeSource = readFile(new URL('./bridge.js', import.meta.url), 'utf8');
...
test('new-folder workflow is wired into the real CloudView and both bridges', async () => {
  const [source, bridge] = await Promise.all([cloudViewSource, bridgeSource]);
  const submitSource = sourceBetween(source, 'async function submitCreateFolder()', 'async function triggerUpload');

  assert.match(source, /import \{ normalizeFolderName, validateFolderName \} from ['"]\.\.\/folderName\.js['"]/);
  assert.match(source, /key:\s*['"]createFolder['"][\s\S]*?label:\s*['"]新建文件夹['"]/);
  assert.match(source, /v-model:open="createFolderDialog\.open"[\s\S]*?title="新建文件夹"/);
  assert.match(submitSource, /bridge\.invoke\(['"]create_folder['"],\s*\{[\s\S]*?parent_id:\s*currentFolderId\.value[\s\S]*?folder_name:\s*folderName/);
  assert.match(submitSource, /if \(createFolderDialog\.loading\) return/);
  assert.match(submitSource, /await loadCloudFiles\(0\)/);
  assert.match(bridge, /command === ['"]create_folder['"][\s\S]*?['"]\/api\/files\/create-folder['"]/);
});

test('desktop upload actions keep folder before file and GCID modal stays viewport-safe', async () => {
  const source = await cloudViewSource;
  const primaryActions = sourceBetween(source, 'class="file-primary-actions"', '<div class="file-list-region"');
  const folderButton = primaryActions.indexOf("triggerUpload('folder')");
  const fileButton = primaryActions.indexOf("triggerUpload('files')");
  const importModal = sourceBetween(source, 'title="导入 GCID JSON"', 'title="选择服务器文件或文件夹"');

  assert.ok(folderButton >= 0, 'missing desktop folder upload button');
  assert.ok(fileButton > folderButton, 'desktop folder upload button must precede file upload');
  assert.match(primaryActions, /<a-dropdown v-else/);
  assert.match(importModal, /\bcentered\b/);
  assert.match(importModal, /wrap-class-name="gcid-import-modal"/);
});
